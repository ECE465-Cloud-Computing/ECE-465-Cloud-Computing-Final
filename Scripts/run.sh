...
sleep 1
ssh -i ${KEY_FILE} ${USER}@${INSTANCES_IPS_ARRAY[${NUM_IPS}-1]} "java -cp ${PROG} edu.cooper.ece465.CoordinatorMain 6666 time 0 3 ${PRIVATE_IPS_ARRAY[0]} ${PRIVATE_IPS_ARRAY[1]} ${PRIVATE_IPS_ARRAY[2]} ${PRIVATE_IPS_ARRAY[3]}" | tee -a ${LOGFILE}
